...
int main()
{
    ...
    if(pid==-1){
        ...
        for (int i = 0; i < 5;i++){
            int res = msgsnd(recv_id, &msg[i], sizeof(Person), 0);
            if(res==-1){
                printf("[child send msg-%d failed]\n", i);
            }else{
                printf("[child send msg-%d success]%s %d\n", i, msg[i].person.name, msg[i].person.age);
            }
        }
        for (int i = 0; i < 5;i++){
            int res = msgrcv(send_id, &msg[i], sizeof(Person), 0, 0); // 最后一个参数，代表默认阻塞读取
            if(res==-1){
                printf("[child recv msg-%d failed]\n", i);
            }else{
                printf("[child recv msg-%d success]%s %d\n", i, msg[i].person.name, msg[i].person.age);
            }
        }
    }
    else
    {
        ...
        for (int i = 0; i < 5;i++){
            ...
            if(res==-1){
                printf("[parent send msg-%d failed]\n", i);
            }else{
                printf("[parent send msg-%d success]%s %d\n", i, msg[i].person.name, msg[i].person.age);
            }
        }
        for (int i = 0; i < 5;i++){
            int res = msgrcv(recv_id, &msg[i], sizeof(Person), 0, 0); // 最后一个参数，代表默认阻塞读取
            if(res==-1){
                printf("[parent recv msg-%d failed]\n", i);
            }else{
                printf("[parent recv msg-%d success]%s %d\n", i, msg[i].person.name, msg[i].person.age);
            }
        }
    }
    return 0;
