...
=================        生成req队列需要的pack的相关操作     ===============================
    ...
    def gen_task_download_upload(self, task_type, f_path, f_name, f_size, f_mtime, md5):
        ...
        if task_type == 'download':
            ...
            # 将需要下载的文件信息写入db
            self.db_insert(relative_path=remote_relative_path, size=f_size, mtime=f_mtime, md5=md5)
...
=================        生成req队列需要的pack的相关操作     ===============================
    ...
    def first_sync(self):
        ...
        if remote_dir_list is not []:
            ...
            for remote_file in remote_dir_list:
                ...
                if not is_swp_file(remote_file['filename']):
    ...
    def not_first_sync(self):
        ...
        for db_file_path_name in db_map.keys():
            # now += 1
            # print('\n..当前{0}，共计{1}'.format(now, num))
            if (not is_swp_file(db_file_path_name)) and (db_file_path_name not in local_file_map.keys()):
                task_dict = {}
        ...
        while self.is_die == 0:
            ...
            if self.sock in rs:
                ...
                if head == 'uploadFileRes' and error == 4:
                    ...
                    """
                    print_info('recver', error, '上传文件返回信息：{}'.format(body_json['msg']))
