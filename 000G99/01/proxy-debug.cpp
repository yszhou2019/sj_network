	...
	for(int i = 0; i < number; i++) {
		...
		if(sockfd == listen_fd) {
            ...
            if( (events[i].events & EPOLLOUT) && (sinfos[sockfd]->S2C_len > 0) ){
                ...
                // TODO DEBUG
                FILE *temp;
                temp = fopen("fromserver", "a+");
                for (int i = 0; i < sinfo->S2C_len;i++){
                    fputc(sinfo->S2C_buf[i], temp);
                }
                fclose(temp);
            ...
            if( (events[i].events & EPOLLOUT) && (sinfos[client_fd]->C2S_len > 0) ){
                FILE *temp;
                temp = fopen("fromclient", "a+");
                for (int i = 0; i < sinfo->C2S_len;i++){
                    fputc(sinfo->C2S_buf[i], temp);
                }
                fclose(temp);
...
int main(int argc, char** argv)
{
    ...
    Choice options[] = {
        {"--proxyport", req_arg, must_appear, is_int, {.int_val=&proxy_port}, 20000, 20009, 20001},
